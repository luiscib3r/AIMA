...
class GraphicEnvironment(XYEnvironment):
    def __init__(self, width=10, height=10, cell_size=25, boundary=True, color={}, display=False):
        """Define all the usual XYEnvironment characteristics,
        but initialise a BlockGrid for GUI too."""
        super().__init__(width, height)

        self.cell_size = cell_size
        self.grid = pygame.display.set_mode((width*cell_size, height*cell_size))
    ...
    def draw_world(self):
        for y in range(0, self.height):
            for x in range(0, self.width):
                poly = [(x * self.cell_size, y * self.cell_size),
                        ((x + 1) * self.cell_size, y * self.cell_size),
                        ((x + 1) * self.cell_size, (y + 1) * self.cell_size),
                        (x * self.cell_size, (y + 1) * self.cell_size)]

                pygame.draw.polygon(self.grid, (200, 200, 200), poly, 0)
                pygame.draw.polygon(self.grid, (100, 100, 100), poly, 1)
        ...
        for x in range(0, len(world)):
            for y in range(0, len(world[x])):
                if len(world[x][y]):
                    ...
                    pygame.draw.polygon(self.grid,  self.colors[world[x][y][-1].__class__.__name__], poly, 0)
